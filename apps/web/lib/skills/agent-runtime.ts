...
type RuntimeResult = { status?: string; skill_id?: string; result?: { message?: string; overview?: string; themes?: Array<{ title?: string; summary?: string }>; sources?: RuntimeSource[]; provider?: string; cached?: boolean }; error_code?: string };

const MIN_SKILL_CONFIDENCE = 0.85;

export async function runMatchedSkill(message: string, locale: "ko" | "en", accessToken: string | null, space: "personal" | "research" = "personal"): Promise<{ message: string; skillId: string; sources?: RuntimeSource[]; cached?: boolean } | null> {
  ...
  try {
    ...
    if (result.result.sources?.length) {
      ...
      return { skillId: decision.skill_id, message, sources, cached: Boolean(result.result.cached) };
    }
