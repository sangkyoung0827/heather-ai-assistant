...
type WorkspaceId = "dashboard" | "chat" | "direct" | "personal" | "researcher" | "memoryArchive" | "projects" | "connections" | "approvals" | "contextImport" | "sensitive" | "local" | "settings";
...
const NODES: Node[] = [
  ...
  { id: "memoryArchive", label: "Memory Archive", detail: "Photos & journal", icon: BookOpen, x: 790, y: 555, path: "/memory-archive" }
];
...
function WorkspacePage({ workspace, pathname, node, data, onNavigate }: { workspace: WorkspaceId; pathname: string; node: Node; data: ReturnType<typeof useHeatherData>; onNavigate: (path: string) => void }) {
  const meta: Record<WorkspaceId, [string, string]> = { dashboard: ["Dashboard", "Heather의 전체 구조와 최근 활동을 확인하세요."], chat: ["채팅", "Heather와 대화하며 작업을 요청하세요."], direct: ["직접명령 등록", "반복되는 질문과 응답을 저장하고 관리하세요."], personal: ["개인 메모리", "개인 정보, 관계, 선호와 결정을 관리하세요."], researcher: ["Researcher", "연구자료 등록과 연구 기록을 관리하세요."], memoryArchive: ["추억 저장소", "사진과 일기로 나의 역사를 기록하는 공간입니다."], projects: ["프로젝트", "프로젝트, 별칭, 리소스를 안전하게 관리하세요."], connections: ["연결", "권한이 필요한 외부 연결을 확인하세요."], approvals: ["승인 센터", "실행 전 확인과 감사 기록을 관리하세요."], contextImport: ["개인 컨텍스트 가져오기", "검토한 seed 항목만 저장합니다."], sensitive: ["민감 메모리", "일반 채팅과 분리된 민감 컨텍스트입니다."], local: ["Local Control", "안전한 로컬 기능을 관리하세요."], settings: ["Settings", "Heather 환경을 설정하세요."] };
