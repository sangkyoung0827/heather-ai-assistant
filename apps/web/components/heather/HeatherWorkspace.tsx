...
import { BrainCircuit, Command, Database, FolderKanban, Home, Laptop, MessageSquare, Settings, Sparkles } from "lucide-react";
...
export function HeatherWorkspace() {
  ...
  return <main className="heather-hud"><div className="heather-app-shell">
    ...
    <div className="heather-main-column">
      {workspace === "dashboard" ? <DashboardHome node={activeNode} projects={activeProjects} memories={activeMemories} conversations={data.conversations.length} onNavigate={navigate} /> : <WorkspacePage workspace={workspace} node={activeNode} data={data} onNavigate={navigate} />}
    </div>
  </div></main>;
}
...
function WorkspacePage({ workspace, node, data, onNavigate }: { workspace: WorkspaceId; node: Node; data: ReturnType<typeof useHeatherData>; onNavigate: (path: string) => void }) {
  ...
  const [title, description] = meta[workspace];
  return <><header className="workspace-topbar"><button type="button" className="workspace-breadcrumb" onClick={() => onNavigate("/dashboard")}><Home className="h-4 w-4" /> Dashboard</button><div><h1>{title}</h1><p>{description}</p></div></header><section className="workspace-canvas">{!data.ready ? <div className="workbench-loading">Heather 작업공간을 불러오는 중입니다.</div> : <>{workspace === "chat" && <ChatPanel conversations={data.conversations} memories={data.memories} projects={data.projects} teachings={data.teachings} automationRecipes={data.automationRecipes} settings={data.settings} onSaveConversation={data.saveConversation} onDeleteConversation={data.deleteConversation} onSaveMemory={data.saveMemory} onSaveSettings={data.saveSettings} />}{workspace === "direct" && <DirectCommandRegistrationPanel />}{workspace === "personal" && <MemoryPanel variant="personal" memories={data.memories} onSaveMemory={data.saveMemory} onDeleteMemory={data.deleteMemory} />}{workspace === "research" && <MemoryPanel variant="research" memories={data.memories} onSaveMemory={data.saveMemory} onDeleteMemory={data.deleteMemory} />}{workspace === "researcher" && <AnalysisPanel memories={data.memories} onSaveMemory={data.saveMemory} />}{workspace === "local" && <LocalControlPanel settings={data.settings} onSaveSettings={data.saveSettings} />}{workspace === "settings" && <SettingsPanel settings={data.settings} onSaveSettings={data.saveSettings} onClearAll={data.clearAll} />}</>}</section></>;
}
