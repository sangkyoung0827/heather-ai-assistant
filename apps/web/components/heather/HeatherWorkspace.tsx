...
import { Bell, BookOpen, BrainCircuit, Command, Database, Factory, FolderKanban, Home, Laptop, MessageSquare, Palette, Settings, Sparkles } from "lucide-react";
...
function GlobalRail({ active, onNavigate, settings }: { active: WorkspaceId; onNavigate: (path: string) => void; settings: ReturnType<typeof useHeatherData>["settings"] }) {
  return <aside className="heather-icon-rail" aria-label="Heather navigation"><button type="button" className="rail-brand" onClick={() => onNavigate("/dashboard")} aria-label="Heather home"><HeatherAvatar settings={settings} size="small" /></button><div className="rail-actions">{NODES.map((node) => <RailButton key={node.id} icon={node.icon} label={node.label} active={active === node.id} onClick={() => onNavigate(node.path)} />)}</div><div className="rail-bottom"><RailButton icon={Laptop} label="Local control" active={active === "local"} onClick={() => onNavigate("/local-control")} /><RailButton icon={Settings} label="Settings" active={active === "settings"} onClick={() => onNavigate("/settings")} /></div></aside>;
}

function DashboardHome({ projects, memories, conversations, onNavigate, settings, onSaveSettings }: { node: Node; projects: ProjectRecord[]; memories: MemoryRecord[]; conversations: number; onNavigate: (path: string) => void; settings: ReturnType<typeof useHeatherData>["settings"]; onSaveSettings: ReturnType<typeof useHeatherData>["saveSettings"] }) {
  ...
  return <section className="dashboard-layout"><aside className="dashboard-brand-panel"><HeatherAvatar settings={settings} size="large" /><div><p className="dashboard-eyebrow">Heather AI Assistant</p><h1>Heather</h1><p>지식과 작업을 연결하는 개인 AI 파트너</p></div><div className="brand-control"><div><strong>아이콘 스타일</strong><small>현재 설정</small></div><div className="icon-style-picker">{(["avatar", "orb", "spark", "neural"] as const).map((style) => <button key={style} type="button" className={settings.iconStyle === style ? "is-active" : ""} onClick={() => void onSaveSettings({ ...settings, iconStyle: style })}><HeatherAvatar settings={{ ...settings, iconStyle: style }} size="small" /></button>)}</div></div><div className="brand-control"><div><strong>Accent color</strong><Palette className="h-4 w-4" /></div><div className="accent-picker">{Object.entries(ACCENT_THEMES).map(([name, theme]) => <button key={name} type="button" title={name} aria-label={name} className={settings.accentColor === name ? "is-active" : ""} style={{ "--swatch": theme.accent } as CSSProperties} onClick={() => void onSaveSettings({ ...settings, accentColor: name as keyof typeof ACCENT_THEMES })}><span /></button>)}</div></div><small className="brand-note">설정은 이 기기에 저장됩니다.</small></aside><section className="dashboard-workspace"><header className="dashboard-header"><div><p className="dashboard-eyebrow">{today}</p><h2>오늘의 작업 공간</h2></div><div><button type="button" className="dashboard-command" onClick={() => onNavigate("/chat")}>Heather에게 요청하기</button><span className="dashboard-connection"><Bell className="h-3.5 w-3.5" /> 연결됨</span></div></header><div className="dashboard-metrics"><DashboardMetric icon={Command} label="직접명령" value="관리" detail="반복 작업 설정" onClick={() => onNavigate("/direct-commands")} /><DashboardMetric icon={Database} label="개인 메모리" value={String(personalMemories)} detail={personalMemories ? "저장된 개인 맥락" : "저장된 메모리가 없습니다"} onClick={() => onNavigate("/memory/personal")} /><DashboardMetric icon={BrainCircuit} label="연구 메모리" value={String(researchMemories.length)} detail={researchMemories.length ? "연구 컨텍스트 사용 가능" : "저장된 연구 메모리가 없습니다"} onClick={() => onNavigate("/researcher/memory")} /><DashboardMetric icon={MessageSquare} label="최근 대화" value={String(conversations)} detail={conversations ? "대화 이어가기" : "아직 저장된 대화가 없습니다"} onClick={() => onNavigate("/chat")} /></div><div className="dashboard-columns"><DashboardPanel title="우선 작업" action="프로젝트 열기" onAction={() => onNavigate("/researcher")}>{projects.length ? <ul>{projects.slice(0, 4).map((project) => <li key={project.id}><span /><div><strong>{project.title}</strong><small>{project.next_actions[0] || "다음 작업이 없습니다."}</small></div><em>{project.status}</em></li>)}</ul> : <EmptyState text="아직 등록된 프로젝트가 없습니다." />}</DashboardPanel><DashboardPanel title="연구 상태" action="Researcher 열기" onAction={() => onNavigate("/researcher")}>{researchMemories.length ? <ul>{researchMemories.slice(0, 4).map((memory) => <li key={memory.id}><span className="research-dot" /><div><strong>{memory.source || "연구 메모리"}</strong><small>{memory.content.slice(0, 65)}</small></div></li>)}</ul> : <EmptyState text="최근 연구자료와 공정 데이터가 없습니다." />}</DashboardPanel><DashboardPanel title="빠른 진입" action="전체 보기" onAction={() => onNavigate("/chat")}><div className="dashboard-shortcuts">{NODES.filter((node) => node.id !== "dashboard").map((node) => <Shortcut key={node.id} node={node} onNavigate={onNavigate} />)}</div></DashboardPanel></div></section></section>;
}
...
function Shortcut({ node, onNavigate }: { node: Node; onNavigate: (path: string) => void }) { const Icon = node.icon; return <button type="button" onClick={() => onNavigate(node.path)}><Icon className="h-4 w-4" />{node.label}</button>; }
